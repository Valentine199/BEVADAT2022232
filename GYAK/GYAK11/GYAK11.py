...
Original file is located at
    https://colab.research.google.com/drive/136lio5D1mGYGu9GLNXswHfLBLmLbhr4x
...
# Az értékei legyenek 0 vagy 1
# dtype legyen np.float32
kernel = np.random.randint(0,2, size=(3,3))
...
for i in range(len(newImg)):
  for j in range(len(newImg[i])):
    newImg[i,j] = np.sum(img[i:kerHeight+i, j:kerWidth+j] *  kernel)
    ...
    def patches_generator(self, image):
        ...
        for h in range(image_h-self.kernel_size+1):
            for w in range(image_w-self.kernel_size+1):
                patch = image[h:self.kernel_size+h, w:self.kernel_size+w] #indexelj ki egy kernelnyi mátrixot a bemeneti képből
